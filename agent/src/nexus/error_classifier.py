...
Ported from Hermes (`agent/error_classifier.py`) with light adaptation:
Nexus has no credential-pool rotation yet, so the `should_rotate_credential`
flags are kept in the output but treated as hints rather than requirements.
The retry loop in `agent.loop` currently honours `retryable`,
`should_compress`, and `should_fallback` — `should_rotate_credential` is
surfaced so future credential-pool code can consume it.
...
Replaces the bare ``raise LLMTransportError(f"HTTP {code}: {body}")``
path in ``agent.llm`` with a rich classification consumed by the
agent loop's retry logic.
"""
...
class ClassifiedError:
    ...
    def user_facing_summary(self) -> str:
        """One-line, non-technical summary for display in chat / toasts."""
        ...
        if self.reason == FailoverReason.auth:
