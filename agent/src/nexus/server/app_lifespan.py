...
def _startup_local_migration(mutable_state: dict[str, Any], agent: Any) -> None:
    try:
...
def _start_vault_cache_listener() -> asyncio.Task[None]:
    ...
    return asyncio.create_task(_vault_cache_listener())


def _startup_tunnel_cleanup() -> None:
    try:
...
async def _startup_local_llm(mutable_state: dict[str, Any], agent: Any) -> None:
...
def _startup_hitl_sweep(sessions: Any) -> None:
    try:
...
async def _startup_graphrag(graphrag_cfg: Any, nexus_cfg: Any) -> None:
...
async def _startup_workflows(app: FastAPI, agent: Any, sessions: Any) -> None:
    try:
...
async def _startup_mcp_manager(nexus_cfg: Any, agent: Any, app: FastAPI) -> Any:
    mcp_manager = None
    try:
...
def _startup_mcp_server_mode(nexus_cfg: Any, agent: Any) -> None:
    try:
...
def _startup_broker_poller(app: FastAPI, agent: Any) -> None:
    try:
...
def _shutdown_memory_store() -> None:
    from ..agent.memory import close_memory_store
    close_memory_store()


def _shutdown_dream_store() -> None:
    try:
...
async def _shutdown_workflows(app: FastAPI) -> None:
    try:
...
async def _shutdown_mcp(mcp_manager: Any) -> None:
...
async def _shutdown_broker(app: FastAPI) -> None:
    try:
...
def create_lifespan(state: dict[str, Any]):
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        _startup_local_migration(mutable_state, agent)
        _setup_nexus_logging()
        _startup_event_bus()
